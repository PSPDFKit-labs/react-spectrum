...
import statelyDocs from 'docs:@react-stately-nutrient/list';
...
hook from `@react-stately-nutrient/list`. The state object should be passed as an option to
...
`@react-stately-nutrient/list` implements a JSX based interface for building collections instead.
