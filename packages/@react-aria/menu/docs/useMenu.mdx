...
import statelyDocs from 'docs:@react-stately-nutrient/menu';
import treeDocs from 'docs:@react-stately-nutrient/tree';
...
hook from `@react-stately-nutrient/menu`. State for the menu itself is managed by the <TypeLink links={treeDocs.links} type={treeDocs.exports.useTreeState} />
hook from `@react-stately-nutrient/tree`. These state objects should be passed to the appropriate React Aria hooks.
...
import type {MenuTriggerProps} from '@react-stately-nutrient/menu';
...
```tsx example
import {Section} from '@react-stately-nutrient/collections';
