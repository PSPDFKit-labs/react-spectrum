...
State is managed by the <TypeLink links={statelyDocs.links} type={statelyDocs.exports.useDateRangePickerState} /> hook from `@react-stately-nutrient/datepicker`. The state object should be passed as an argument to `useDateRangePicker`.
