...
import statelyDocs from 'docs:@react-stately-nutrient/datepicker';
...
State is managed by the <TypeLink links={statelyDocs.links} type={statelyDocs.exports.useTimeFieldState} /> hook from `@react-stately-nutrient/datepicker`. The state object should be passed as an option to `useTimeField` and `useDateSegment`.
