...
import statelyDocs from 'docs:@react-stately-nutrient/select';
...
hook from `@react-stately-nutrient/select`. The state object should be passed as an option to `useSelect`
...
`@react-stately-nutrient/select` implements a JSX based interface for building collections instead.
