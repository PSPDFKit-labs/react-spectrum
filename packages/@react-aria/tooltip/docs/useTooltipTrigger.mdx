...
import statelyDocs from 'docs:@react-stately-nutrient/tooltip';
...
hook in `@react-stately-nutrient/tooltip`. The state object should be passed as an option to `useTooltipTrigger` and `useTooltip`.
