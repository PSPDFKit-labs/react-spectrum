...
State is managed by the <TypeLink links={statelyDocs.links} type={statelyDocs.exports.useRangeCalendarState} /> hook from `@react-stately-nutrient/calendar`. The state object should be passed as an option to `useRangeCalendar`, `useCalendarGrid`, and `useCalendarCell`.
...
```jsx
import {useRangeCalendarState} from '@react-stately-nutrient/datepicker';
import {useLocale} from '@react-aria-nutrient/i18n';
import {GregorianCalendar} from '@internationalized/date';
